...
5. Controllers must not access Mapper / DAO directly. HTTP semantics must not leak into Service / Manager. Lower layers must not depend back on upper layers. For cross-module calls, prefer the other module's Service / Facade. Introduce Manager only when needed.
